...
// Ambil data untuk hapus file (prepared statement)
$result = db_query("SELECT gambar FROM galeri WHERE id = ?", [$id]);
if ($result && mysqli_num_rows($result) > 0) {
    $row = mysqli_fetch_assoc($result);

    if (!empty($row['gambar'])) {
        $file_path = __DIR__ . '/../../assets/img/galeri/' . $row['gambar'];
...
$ok = db_query("DELETE FROM galeri WHERE id = ?", [$id]);
